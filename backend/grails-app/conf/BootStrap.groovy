...
class BootStrap {
    ...
    def initDevelopmentModeData() {
        ...
        (1..2).each { index ->
            event1.addToPdtps(new Pdtp(place: "1-place$index", fromDate: nowDate.toDate(), toDate: nowDate.toDate(),
        ...
        (1..2).each { index ->
            event2.addToPdtps(new Pdtp(place: "2-place$index", fromDate: nowDate.minusDays(index).toDate(),
                toDate: nowDate.plusDays(index).toDate(),
                startTime: time.plusHours(index).toDate(), price: "2-price$index",
                timeDescription: "timeDescription $index"))
        }
        event2.save()
    }
}
